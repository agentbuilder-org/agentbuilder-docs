...
The models in Standard Agents produced by AgentBuilder define the LLM configurations that your agents and prompts use for AI responses. Each model configuration specifies which provider to use, the specific model ID, fallback models for resilience, and pricing for cost tracking.
...
AgentBuilder supports two model providers for the Standard Agents it produces: **OpenAI** (direct) and **OpenRouter** (access to multiple providers).
...
After defining models, AgentBuilder generates the `StandardAgents.Models` type:
